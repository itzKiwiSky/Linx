...
function table.pack(t, drop, indent)
	...
	for k, v in pairs(t) do
		...
		elseif tk == "table" then k = "["..table.pack(k, drop, indent and indent+1).."]"
		...
		elseif tv == "table" then v = table.pack(v, drop, indent and indent+1)
...
function table.unpack(s)
	...
	assert(loadstring("table.table="..s))()
	local t = table.table
	table.table = nil
	return t
end
